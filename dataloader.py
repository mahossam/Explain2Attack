...
def read_corpus(path, clean=True, MR=True, encoding='utf8', shuffle=False, lower=True, fix_labels=False, max_length=-1, labels_only=False):
    data = []
    labels = []
...
def create_batches(x, y, batch_size, map2id, perm=None, sort=False, log_file=None, max_len=-1):
    ...
    for i in range(nbatch):
        bx, by = create_one_batch(x[i*size:(i+1)*size], y[i*size:(i+1)*size], map2id, max_len=max_len)
        batches_x.append(bx)
        batches_y.append(by)
    ...
    sys.stdout.write("{} batches, avg sent len: {:.1f}\n".format(nbatch, sum_len/len(x)))
    if log_file is not None:
        log_file.write("{} batches, avg sent len: {:.1f}\n".format(nbatch, sum_len/len(x)))
        log_file.flush()
...
# shuffle training examples and create mini-batches
def create_batches_x(x, batch_size, map2id, perm=None, sort=False, max_len=-1):
    ...
    nbatch = (len(x)-1) // size + 1
    for i in range(nbatch):
        bx = create_one_batch_x(x[i*size:(i+1)*size], map2id, max_len=max_len)
        sum_len += len(bx)
        batches_x.append(bx)
