...
class Model(nn.Module):
    ...
    def text_pred(self, text, batch_size=32, max_len=-1):
        batches_x = dataloader.create_batches_x(text, batch_size, self.word2id, max_len=max_len)
        outs = []
        with torch.no_grad():
            for x in batches_x:
                x = Variable(x)
...
def train_model(epoch, model, optimizer,
    ...
    if test_acc > best_test:
        ...
        print(f"Test acc not improved from {best_test}")
        log_file.write(f"Test acc not improved from {best_test}\n")
    sys.stdout.write("\n")
    log_file.write("\n")
    log_file.flush()
    return best_test
...
def main(args):
    ...
    #     test_x, test_y = dataloader.read_corpus('/data/medg/misc/jindi/nlp/datasets/mr/test.txt', max_length=max_length)
        train_x, train_y = dataloader.read_corpus('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/mr/train.txt', max_length=max_length)
        test_x, test_y = dataloader.read_corpus('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/mr/test.txt', max_length=max_length)
    elif args.dataset == 'imdb':
        train_x, train_y = dataloader.read_corpus(os.path.join('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/imdb','train_tok.csv'), clean=False, MR=True, shuffle=False, max_length=max_length)
        test_x, test_y = dataloader.read_corpus(os.path.join('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/imdb','test_tok.csv'), clean=False, MR=True, shuffle=False, max_length=max_length)
    else:
        fix_labels = False
        if args.dataset == "yelp" or args.dataset == "fake" or args.dataset == "ag":
            fix_labels = True

        train_x, train_y = dataloader.read_corpus('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/{}/'
                                                    'train_tok.csv'.format(args.dataset),
                                                  clean=False, MR=True, shuffle=False, fix_labels=fix_labels, max_length=max_length)
        test_x, test_y = dataloader.read_corpus('/home/mahmoudm/pb90_scratch/mahmoud/TextFooler-master/data/adversary_training_corpora/{}/'
    ...
    log_file = open(os.path.join(os.path.dirname(args.save_path), f'{os.path.basename(args.save_path)}.log'), 'a')
    ...
    train_x, train_y = dataloader.create_batches(
        train_x, train_y,
        args.batch_size,
        model.word2id, max_len=max_length)
    ...
    lengths = np.array([len(seq) for batch in train_x for seq in batch.t().contiguous()])
    ...
    for epoch in range(args.max_epoch):
        best_test = train_model(epoch, model, optimizer,
            ...
            # valid_x, valid_y,
            test_x, test_y,
            best_test, args.save_path, log_file)
        if args.lr_decay>0:
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--max_length", type=int, required=True, default=-1)
